...
from QGT import train_on_all_configs_QGT
#--
...
parser.add_argument('--cost', type=str, default='infidelity',
                    help='cost function')
...
lr = 0.01
num_epochs = 15000
features = get_features(N,extra_large=False)
...
print(f"Using {cost_fxn_name} cost function + QGT and training for {num_epochs} epochs")
...
path = base_path + system_path +  f'FullEnumQGT/{cost_fxn_name}/'
weights_path = base_path + f'weights/' + system_path + f'FullEnumQGT/{cost_fxn_name}/'
if batch_size != 2**N:
...
for i, (depth,width) in enumerate(features):
    if features[i] not in train_losses_dict.keys():
        ...
        if num_params_dict[features[i]] < 10000:
            print('Able to perform training with QGT')
            train_losses, fidelities, final_params = train_on_all_configs_QGT(nn_features, num_epochs, batch_size, cost_fxn, 
                                                                            x_train, y_train, all_target_logpsis,
                                                                            key=seed, lr=lr,
                                                                            PRINT=TRAIN_PRINT)
